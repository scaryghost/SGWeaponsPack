class SGWPBuyMenuInvList extends SRKFBuyMenuInvList;
...
function UpdateMyBuyables() {
    ...
    for ( CurInv = PlayerOwner().Pawn.Inventory; CurInv != none; CurInv = CurInv.Inventory ) {
        if ( KFWeapon(CurInv) != none ) {
            if ( KFWeapon(CurInv).default.PickupClass == class'SGWeaponsPack.DualDeaglePickup' ) {
                bHasDualCannon = true;
            }

            if ( KFWeapon(CurInv).default.PickupClass == class'KFMod.DualiesPickup' ) {
                bHasDual = true;
            }

            if ( KFWeapon(CurInv).default.PickupClass == class'SGWeaponsPack.Dual44MagnumPickup' ) {
                bHasDual44 = true;
            }
        }
    }
    ...
    for ( CurInv = PlayerOwner().Pawn.Inventory; CurInv != none; CurInv = CurInv.Inventory ) {
        ...
        if ( (bHasDual && MyPickup == class'KFMod.SinglePickup') ||
             (bHasDualCannon && MyPickup == class'SGWeaponsPack.DeaglePickup') ||
             (bHasDual44 && MyPickup == class'SGWeaponsPack.Magnum44Pickup') ) {
            continue;
        }
        ...
        if ( CurInv.IsA('KFWeapon') ) {
            ...
            if ( KFWeapon(CurInv).bHasSecondaryAmmo ) {
                ...
                MyBuyable.ItemCost          = (float(MyPickup.default.Cost) * PlayerVeterancy.static.GetCostScaling(KFPRI, MyPickup)) / DualDivider;
                MyBuyable.ItemAmmoCost      = MyPickup.default.AmmoCost * PlayerVeterancy.static.GetAmmoCostScaling(KFPRI, MyPickup) * PlayerVeterancy.static.GetMagCapacityMod(KFPRI, KFWeapon(CurInv));
                if( MyPickup == class'SGWeaponsPack.HuskGunPickup' ) {
